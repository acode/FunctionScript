...
class Gateway extends EventEmitter {
  ...
  formatRequest(req) {
    return `(${req ? [(req._background ? 'background:' : '') + req._uuid, req.connection.remoteAddress].join(' ') : 'GLOBAL'}) ${this.routename(req)}`;
  }
  ...
  __httpHandler__(req, res) {
    ...
    if (req.headers['user-agent'] && !urlinfo.pathname.endsWith('/') && urlinfo.pathname.split('/').pop().indexOf('.') === -1) {
    ...
    req.on('data', chunk => {
      ...
      if (size > this.maxRequestSizeMB * 1024 * 1024) {
        this.log(req, `<!> ClientError: Function Payload Exceeded, Max Size ${this.maxRequestSizeMB}MB`);
    ...
    req.on('end', () => {
      ...
      this.resolve(req, res, buffer, (err, definition, data, buffer) => {
        ...
        if (err) {
          this.log(req, `<!> ClientError: ${err.message}`);
  ...
  __requestHandler__(req, res, definition, data, buffer) {
    ...
    this.execute(definition, functionArgs, data, (err, value, headers) => {
      ...
      if (err) {
        if (err.fatal) {
          this.log(req, `<!> Fatal Error (${dt}ms): ${err.message}`);
          return this.__fatalError__(req, res, err.message);
        } else {
          this.log(req, `<!> Runtime Error (${dt}ms): ${err.message}`);
        ...
        this.log(req, `<!> Value Error (${dt}ms): ${details.returns.message}`);
