...
class Gateway extends EventEmitter {
  ...
  __parseParameters__ (contentType, contentTypeParameters, convert, query, buffer, definition) {
  ...
  __validateParameters__ (parsed, definition) {
    ...
    let paramsList = definition.params.map(param => {
      ...
      } else if (!types.validate(param.type, value, nullable, param.schema)) {
  ...
  __httpHandler__ (req, res) {
    ...
    req.on('end', () => {
      ...
      this.resolve(req, res, buffer, (err, definition, data, buffer) => {
        ...
        try {
          parsed = this.__parseParameters__(contentType, contentTypeParameters, convert, query, buffer, definition);
        } catch (e) {
  ...
  __requestHandler__ (req, res, definition, data, functionArgs) {
    ...
    this.execute(definition, functionArgs, data, (err, value, headers) => {
      ...
      } else if (!types.validate(definition.returns.type, value, false, definition.returns.schema)) {
