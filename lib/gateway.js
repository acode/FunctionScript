...
class Gateway extends EventEmitter {
  ...
  constructor(cfg) {
    ...
    this.supportedBgModes = {
      'info': (req, definition, params) => new Buffer(`Initiated ${definition.name}...`),
      'empty': (req, definition, params) => new Buffer(0),
      'params': (req, definition, params) => params
    };
    this.defaultBgMode = 'info';
    this.server = null;
    this.definitions = {};
    this._requests = {};
  ...
  __background__(req, res, definition, params) {
    let value = req._background(req, definition, params);
    let headers = this.__createHeaders__(req);
    headers['content-type'] = headers['content-type'] ||
      (value instanceof Buffer ? 'text/plain' : 'application/json');
  ...
  __httpHandler__(req, res) {
    ...
    if ('bg' in pathquery) {
      req._background = this.supportedBgModes[pathquery['bg']] ||
