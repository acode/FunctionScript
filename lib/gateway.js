...
class Gateway extends EventEmitter {
  ...
  __requestHandler__ (req, res, definition, context, functionArgs) {
    let nullable = (definition.returns || {}).defaultValue === null;
    this.log(req, `Execution Start`);
    let t = new Date().valueOf();
    this.execute(definition, functionArgs, context, (err, value, headers, executionUuid) => {
  ...
  execute (definition, functionArgs, context, callback) {
    let fn;
    ...
    if (definition.format.language === 'static') {
      ...
      if (contentType.split(';')[0].split('/')[0] === 'video') {
        let range = context.http.headers.range;
      ...
      if (definition.format.async) {
        ...
        try {
          fn.apply(null, functionArgs.concat((err, result, headers) => {
            return callback(err, result, headers, executionUuid);
          }));
        } catch (e) {
          if (!(e instanceof Error)) {
            let value = e;
      ...
      return callback(new Error(`Gateway does not support language "${definition.format.language}"`))
    }
  }
