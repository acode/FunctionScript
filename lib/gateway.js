...
const FUNCTION_EXECUTION_TIMEOUT = 1000;
...
class Gateway extends EventEmitter {
  ...
  __autoformat__ (httpResponse) {
    if ((httpResponse.headers['content-type'] || '').split(';')[0] === 'text/x-scss') {
      let body = httpResponse.body;
      body = sass.renderSync({data: body.toString(), outputStyle: 'compressed'}).css;
      let suffix = ((httpResponse.headers['content-type'] || '').split(';')[1] || '');
      httpResponse.headers['content-type'] = 'text/css' + (suffix.trim() ? `;${suffix}` : '');
      httpResponse.headers['content-length'] = body.byteLength;
      httpResponse.headers['x-autoformat'] = 'true';
  ...
  execute (definition, functionArgs, data, callback) {
    ...
    setTimeout(() => {
      let error = new Error(`Timeout of ${FUNCTION_EXECUTION_TIMEOUT}ms exceeded.`);
