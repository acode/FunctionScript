...
class Gateway extends EventEmitter {
  ...
  __accessSourceError__ (req, res, msg) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    res.finished || res.writeHead(401, this.__formatHeaders__(headers));
    return this.__endRequest__(req, res, JSON.stringify({
      error: {
  ...
  __paymentRequiredError__ (req, res, msg) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    res.finished || res.writeHead(402, this.__formatHeaders__(headers));
    return this.__endRequest__(req, res, JSON.stringify({
      error: {
  ...
  __rateLimitError__ (req, res, count, period) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    res.finished || res.writeHead(429, this.__formatHeaders__(headers));
    return this.__endRequest__(req, res, JSON.stringify({
      error: {
        type: 'RateLimitError',
  ...
  __parseParameters__ (contentType, contentTypeParameters, convert, query, buffer, definition, proxyParameters) {
    ...
    if (!contentType) {
      ...
      try {
        let parsedQuerystring = querystring.parse(buffer.toString());
        params = Object.keys(parsedQuerystring).reduce((params, paramName) => {
          params[paramName] = parsedQuerystring[paramName];
          return params;
        }, {});
        buffer = Buffer.from([]);
        convert = true;
  ...
  __httpHandler__ (req, res) {
    ...
    req.on('end', () => {
      ...
      this.resolve(req, res, buffer, (err, definition, data, buffer, proxyParameters) => {
        ...
        let query = Object.keys(urlinfo.query).reduce((query, paramName) => {
          query[paramName] = urlinfo.query[paramName];
          return query;
        }, {});
        let parsed;
  ...
  createContext (req, definition, params, data, buffer) {
    ...
    if (
      ...
      params.hasOwnProperty('__providers') &&
      typeof params.__providers === 'object'
    ) {
      context.providers = params.__providers;
    }
    return context;
  }
