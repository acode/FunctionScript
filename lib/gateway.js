...
class Gateway extends EventEmitter {
  ...
  __rateLimitError__ (req, res, count, period) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    return this.__endRequest__(
      429,
      this.__formatHeaders__(headers),
      ...
      JSON.stringify({
        error: {
          type: 'RateLimitError',
          message: `Too many requests. The rate limit for this endpoint is ${count} requests in ${period} seconds.`,
          details: {
            rate: {count, period}
          }
        }
  ...
  __fatalError__ (req, res, msg, stack, executionUuid) {
    ...
    let headers = this.__createHeaders__(req, {'content-type': 'application/json', 'x-execution-uuid': executionUuid});
    return this.__endRequest__(
      500,
      this.__formatHeaders__(headers),
      req,
  ...
  __runtimeError__ (req, res, msg, stack, executionUuid) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json', 'x-execution-uuid': executionUuid});
    return this.__endRequest__(
      403,
      this.__formatHeaders__(headers),
      ...
      this.resolve(req, res, buffer, (err, definition, data, buffer, proxyParameters) => {
        ...
        if (err) {
          if (err.accessSourceError) {
            ...
            return this.__rateLimitError__(req, res, err.rate && err.rate.count, err.rate && err.rate.period);
    ...
    this.execute(definition, functionArgs, data, (err, value, headers, executionUuid) => {
      ...
      if (err !== null) {
        if (!(err instanceof Error)) {
          ...
          return this.__runtimeError__(req, res, msg, executionUuid);
        } else if (err.thrown) {
          ...
          return this.__runtimeError__(req, res, message, err.stack, executionUuid);
        } else if (err.fatal) {
          ...
          return this.__runtimeError__(req, res, err.message, err.stack, executionUuid);
        }
      } else if (
        !types.validate(
  ...
  execute (definition, functionArgs, data, callback) {
    ...
    if (definition.format.inline) {
      ...
      if (!fn) {
        try {
          ...
          e.fatal = true;
          return callback(e, null, null, executionUuid);
        }
      }
    } else {
      try {
    ...
    process.once('unhandledRejection', (err, p) => callback(err, null, null, executionUuid));
    if (definition.format.async) {
      fn.apply(null, functionArgs)
        .then(result => callback(null, result, null, executionUuid))
        .catch(e => {
          if (!(e instanceof Error)) {
            let value = e;
            e = new Error(e || '');
          ...
          e.thrown = true;
          callback(e, null, null, executionUuid);
        });
    } else {
      try {
        fn.apply(null, functionArgs.concat((err, results, headers) => {
          return callback(err, results, headers, executionUuid);
        }));
      } catch (e) {
        if (!(e instanceof Error)) {
          let value = e;
