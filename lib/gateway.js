...
class Gateway extends EventEmitter {
  ...
  __subscriptionUpgradeRequiredError__ (req, res, msg) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    return this.__endRequest__(
      401,
      this.__formatHeaders__(headers),
      ...
      JSON.stringify({
        error: {
          type: 'SubscriptionUpgradeRequiredError',
          message: msg
        }
      })
    );
    ...
    req.on('end', () => {
      ...
      this.resolve(req, res, buffer, (err, definition, data, buffer, proxyParameters) => {
        ...
        if (err) {
          ...
          } else if (err.subscriptionUpgradeRequiredError) {
            return this.__subscriptionUpgradeRequiredError__(req, res, err.message);
