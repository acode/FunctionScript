...
class Gateway extends EventEmitter {
  ...
  __complete__ (req, res, value, headers, statusCode) {
    headers = this.__createHeaders__(req, headers);
    headers['content-type'] = headers['content-type'] ||
      (value instanceof Buffer ? 'application/octet-stream' : 'application/json');
    value = value instanceof Buffer ? value : JSON.stringify(value);
    statusCode = statusCode || headers.status;
    delete headers.status;
    res.finished || res.writeHead(statusCode || 200, headers);
  ...
  __requestHandler__ (req, res, definition, data, functionArgs) {
    ...
    this.execute(definition, functionArgs, data, (err, value, headers) => {
      ...
      if (err !== null) {
      ...
      } else if (types.http.validate(definition.returns.type)) {
        let returnType = definition.returns.type;
        let httpObject;
        try {
          httpObject = types.http.parse(value);
        } catch (e) {
          let details = {
            returns: {
              message: `${e.message} (${returnType})`,
              invalid: true,
              expected: {
                type: returnType
              },
        ...
        return this.__complete__(req, res, httpObject.body, httpObject.headers, httpObject.statusCode);
      } else {
        this.log(req, `Execution Complete (${dt}ms)`);
