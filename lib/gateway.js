...
class Gateway extends EventEmitter {
  ...
  __parameterParseError__ (req, res, msg) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    return this.__endRequest__(
      400,
      this.__formatHeaders__(headers),
  ...
  __ownerSuspendedError__ (req, res, msg) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    return this.__endRequest__(
      401,
      this.__formatHeaders__(headers),
  ...
  __ownerPaymentRequiredError__ (req, res, msg) {
    let headers = this.__createHeaders__(req, {'content-type': 'application/json'});
    return this.__endRequest__(
      401,
      this.__formatHeaders__(headers),
  ...
  createContext (req, definition, params, data, buffer) {
    ...
    if (definition.streams) {
      context.stream = this.__stream__.bind(this, definition.streams, context.uuid);
    }
    return context;
  }

  __stream__ (streams, executionUuid, eventName, value) {
    let stream = streams.find(param => param.name === eventName);
    if (!stream) {
      throw new Error([
        `No such stream "${eventName}" in function definition.`,
        ` Please use the syntax "@streams {string} name description"`,
    ...
    if (
      ...
      if (!details[`stream.${eventName}`].mismatch) {
        delete details[`stream.${eventName}`].mismatch;
      }
      if (stream.schema) {
        details[`stream.${eventName}`].expected.schema = stream.schema;
        if (stream.alternateSchemas) {
          details[`stream.${eventName}`].expected.alternateSchemas = stream.alternateSchemas;
        }
      } else if (stream.members) {
        details[`stream.${eventName}`].expected.members = stream.members;
      }
      let req = this._requests[executionUuid];
      this.log(req, `Stream Error: ${details[`stream.${eventName}`].message}`, 'error');
      let error = new Error([
    ...
    this.createServerSentEvent(executionUuid, eventName, JSON.stringify(value));
  }

  createServerSentEvent (executionUuid, eventName, value) {
    let sseInstance = this._serverSentEvents[executionUuid];
    if (sseInstance) {
      let i = 0;
      let timeString = new Date().toISOString();
      let dateString = [timeString, i].join('/');
      while (sseInstance.ids[dateString]) {
        dateString = [timeString, ++i].join('/');
      }
      sseInstance.ids[dateString] = true;
