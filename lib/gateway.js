...
class Gateway extends EventEmitter {
  ...
  __parseParamsFromEncodedURL__ (str) {
    ...
    let params = Object.keys(rawParams).reduce((params, paramName) => {
      let arrayMatches = paramName.match(/^(.*?)\[(.*?)\]$/);
      let objectMatches = paramName.match(/^(.*)\.(.*?)$/);
      if (arrayMatches) {
        let name = arrayMatches[1];
        let index = arrayMatches[2];
        arrayData[name] = arrayData[name] || {indexed: [], pushed: []};
        if (index) {
          let n = parseInt(index);
          if (isNaN(n) || n !== parseFloat(index)) {
            throw new Error(`${name}: Array indices in URL encoded values must be integer values`);
          } else if (parseInt(index) < 0) {
            throw new Error(`${name}: Array indices in URL encoded values must be > 0`);
          } else if (parseInt(index) > 65535) {
            throw new Error(`${name}: Array indices in URL encoded values limited to 65535`);
          }
          arrayData[name].indexed = arrayData[name].indexed.concat(
            Array(Math.max(0, index - arrayData[name].indexed.length)).fill(null)
          );
          arrayData[name].indexed[n] = rawParams[paramName];
        } else {
          arrayData[name].pushed = rawParams[paramName];
        }
      } else if (objectMatches) {
        let name = objectMatches[1];
        let key = objectMatches[2];
        objectData[name] = objectData[name] || {};
        objectData[name][key] = rawParams[paramName];
      } else {
        params[paramName] = rawParams[paramName];
      }
      return params;
    }, {});
    params = Object.keys(arrayData).reduce((params, paramName) => {
      if (paramName in params) {
        throw new Error(`${paramName}: Can not provide both ${paramName} and ${paramName}[] for URL encoded values`);
      }
      params[paramName] = [].concat(arrayData[paramName].indexed, arrayData[paramName].pushed);
      return params;
    }, params);
    params = Object.keys(objectData).reduce((params, paramName) => {
      if (paramName in params) {
        throw new Error(`${paramName}: Can not provide both ${paramName} and ${paramName}.field for URL encoded values`);
      }
      params[paramName] = objectData[paramName];
      return params;
    }, params);
    return params;
  }
