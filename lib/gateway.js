...
class Gateway {
  ...
  __validateParameters__(parsed, definition) {
    ...
    let paramsList = definition.params.map(param => {
      ...
      if (value === undefined) {
        ...
        errors[param.name] = {
          message: `invalid value: ${JSON.stringify(value, null, 2)} (${type}), expected (${param.type})`,
          invalid: true,
          expected: {
            type: param.type
          },
  ...
  __httpHandler__(req, res) {
    ...
    if (!routename.endsWith('/') && urlinfo.pathname.split('/').pop().indexOf('.') === -1) {
      this.log(`${routename}\n::: Redirect`);
    ...
    req.on('data', chunk => {
      ...
      if (size > this.maxRequestSizeMB * 1024 * 1024) {
        ...
        return this.__gatewayError__(req, res, `Function Payload Exceeded, Max Size ${this.maxRequestSizeMB}MB`, 413);
      }
      buffers.push(chunk);
    });
  ...
  __requestHandler__(req, res, definition, data, buffer) {
    ...
    this.execute(definition, functionArgs, data, (err, value, headers) => {
      ...
      if (err) {
        ...
        let details = {
          returns: {
            message: `invalid return value: ${JSON.stringify(value, null, 2)} (${type}), expected (${returnType})`,
            invalid: true,
            expected: {
              type: returnType
            },
  ...
  createContext(req, definition, params, data) {
    ...
    context.http.pathname = urlinfo.pathname;
    return context;
  }
