...
class FunctionParser {
  ...
  parseDefinition (pathname, buffer, functionsPath, isStatic) {
    ...
    if (isStatic) {
      // Convert x.html -> mydomain.com/x
      // But also preserve mydomain.com/x.html
      // Special rules for html files named "index" or "404"
      // Which will create default routes
      let parser = this.getParser('');
      let definition;
      // Should we have a reference for both /hello.html and /hello to the same doc?
      let shouldDuplicateHTMLEntry = true;
      if (ext === 'htm' || ext === 'html') {
        if (name === 'index') {
          name = names.join('/').substr(functionsPath.length);
        } else if (name === '404') {
          name = `${names.join('/').substr(functionsPath.length)}:notfound`;
          shouldDuplicateHTMLEntry = false; // 404 will trigger 404 anyway
        } else {
          name = names.concat(name).join('/').substr(functionsPath.length);
        }
        try {
