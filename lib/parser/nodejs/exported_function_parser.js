    ...
    return functionParams.map((param, i) => {
      if (!commentParams.length) {
        ...
        if (
          ...
          defParam.options &&
          defParam.options.values &&
          !types.validate(defParam.type, param.defaultValue, true, defParam.schema, defParam.options.values)
        ) {
          throw new Error(`Fuction parameter default "${param.defaultValue}" for "${param.name}" is not a valid specified option (${JSON.stringify(defParam.options.values)})`);
        }
