        ...
        .map((param, i) => {
          ...
          if (param.type === 'Identifier') {
            ...
            if (!this.validateFunctionParamName(param.name)) {
              throw new Error(`Invalid parameter name "${param.name}". Function parameter names must contain only alphanumeric characters and underscores.`)
            }
            ...
            if (!this.validateFunctionParamName(param.left.name)) {
              throw new Error(`Invalid parameter name "${param.left.name}". Function parameter names must contain only alphanumeric characters and underscores.`)
            }
    ...
    return functionParams.map((param, i) => {
      if (!commentParams.length) {
        ...
        if (param.type && param.type !== types.defaultType) {
          if (defParam.type !== param.type && !types.validate(defParam.type, param.defaultValue, null, defParam.members)) {
            if (defParam.members) {
              throw new Error(`Fuction parameter default "${param.defaultValue}" for "${param.name}" is not a member of the enum`);
            } else {
