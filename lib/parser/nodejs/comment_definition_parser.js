...
class CommentDefinitionParser {
  ...
  getParameter (values, hasName, schema) {
    ...
    if (schema) {
      ...
      param.schema = this.parseSchema(schema);
    }
    return param;
  }
  ...
  createDefinition (definition, data) {
    ...
    try {
      ...
      if (field === 'description') {
        ...
        definition.params.push(this.getParameter(values, true, schema));
      } else if (field === 'returns') {
        definition.returns = this.getParameter(values, false, schema);
      } else if (field === 'bg') {
  ...
  parseSchema (schema) {
    ...
    return schema.reduce((params, line) => {
      ...
      return params;
    }, []).map((data) => this.getParameter(data.values, true, data.schema));
