  ...
  getParameter (values, schema) {
    ...
    if (options) {
      if (types.optionsAllowed.indexOf(param.type) === -1) {
        throw new Error(`Options {?}: Not allowed for type ${param.type}`);
      }
      param.options = {};
      ...
      if (Array.isArray(values)) {
        ...
        values.forEach(v => {
          var type = param.type === 'object.keyql.query'
            ? 'string'
            : param.type;
          if (!types.validate(type, v)) {
            throw new Error(`Options {?}: "${options}" type mismatch for type ${type}`);
          }
        });
