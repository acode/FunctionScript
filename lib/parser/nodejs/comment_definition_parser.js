  ...
  reduceLines (semanticsList, line) {
    ...
    if (line[0] === '@') {
      ...
      if (previous) {
        ...
        let isEnum = lastSchemaItem && /^{enum}/i.test(lastSchemaItem);
        // Enums inside schemas need to be collected as a single item
        if (isEnum) {
          lastSchemaItem = Array.isArray(lastSchemaItem)
            ? lastSchemaItem.concat(line.trim())
            : [lastSchemaItem, line.trim()];
           previous.schema[previous.schema.length - 1] = lastSchemaItem;
        } else {
