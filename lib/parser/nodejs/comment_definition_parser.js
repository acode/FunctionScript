...
class CommentDefinitionParser {
    ...
    if (semantics.value[0] !== '@') {
      ...
      if (!matches) {
        ...
        if (field === 'returns' || field === 'param') {
          ...
          return this.getFloatValue(semantics);
        } else {
          return semantics;
        }
      }
  ...
  getFloatValue(semantics) {
    let value = parseFloat(semantics.value.trim());
    if (isNaN(value)) {
      throw new Error(`Invalid float value for ${semantics.field}: ${semantics.value}`);
    }
    semantics.value = value;
    return semantics;
  }
  ...
  createDefinition(definition, data) {
    if (data.field === 'description') {
      ...
      // Handle all other cases, like charge
      definition[data.field] = data.value;
    }
