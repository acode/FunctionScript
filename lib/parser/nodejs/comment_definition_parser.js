...
class CommentDefinitionParser {
  ...
  getParameter (values, schema) {
    ...
    if (options) {
      ...
      if (param.type === 'array') {
        if (!param.schema.length) {
          throw new Error(`Options {?}: Can not be added to "${param.name}", please specify an array schema.`);
        }
        var arrayType = param.schema[0].type;
        if (types.optionsAllowed.indexOf(arrayType) === -1) {
          throw new Error(`Options {?}: Not allowed for "${param.name}" array schema type "${arrayType}"`);
        }
      }
      param.options = {};
      ...
      if (Array.isArray(values)) {
        ...
        var paramType = param.type === 'array'
          ? param.schema[0].type
          : param.type;
        paramType = paramType === 'object.keyql.query'
          ? 'string'
          : paramType;
        values.forEach(v => {
          if (!types.validate(paramType, v)) {
            throw new Error(`Options {?}: "${options}" type mismatch for type ${paramType}`);
          }
        });
