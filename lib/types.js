      ...
      if (!!match) {
        ...
        if (commonType === 'array' || commonType === 'object') {
          mergedSchema.push({
            name: s1.name,
            type: commonType,
            schema: _mergeSchemas(commonType, s1.schema, match.schema),
          });
        } else {
          mergedSchema.push({
            name: s1.name,
            type: commonType,
            sampleValue: s1.sampleValue
          });
        }
      }
      return mergedSchema;
  ...
  if (type === 'array') {
    if (!v.length) {
      ...
      let schema = v.reduce((commonModel, entry) => {
        ...
        let commonType = commonModel.type;
        if (commonModel.type !== entryData.type) {
          commonType = 'any';
        }
        if (commonType === 'object' || commonType === 'array') {
          return {
            type: commonType,
            schema: _mergeSchemas(commonType, commonModel.schema, entryData.schema)
          };
        } else {
          return {
            type: commonType
