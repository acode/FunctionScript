...
const validate = (type, v, nullable, schema, options) => {
  return (nullable && v === null) ||
    _validations[type](v, schema, options);
};

const _validations = {
  string: (v, schema, options = []) => typeof v === 'string' && (options.length ? options.indexOf(v) > -1 : true),
  number: (v, schema, options = []) => typeof v === 'number' && v === v && (options.length ? options.indexOf(v) > -1 : true),
  float: (v, schema, options = []) => parseFloat(v) === v && (options.length ? options.indexOf(v) > -1 : true),
  integer: (v, schema, options = []) => parseInt(v) === v && v >= Number.MIN_SAFE_INTEGER && v <= Number.MAX_SAFE_INTEGER && (options.length ? options.indexOf(v) > -1 : true),
      ...
      if (param) {
        for (let i = 0; i < v.length; i++) {
          let invalidParam = !validate(
            ...
            v[i],
            param.defaultValue === null,
            param.type === 'enum' ? param.members : param.schema,
            options
          );
          if (invalidParam) {
            return false;
  ...
  'object.keyql.query': (v, schema, options = []) => {
    try {
