...
const _validations = {
    ...
    if (!!v && typeof v === 'object' && !Array.isArray(v) && !Buffer.isBuffer(v)) {
      for (let i = 0; i < schema.length; i++) {
        ...
        let invalidParam = !validate(
          param.type,
          v.hasOwnProperty(param.name) ? v[param.name] : param.defaultValue,
  ...
  'object.keyql.query': v => _validations.object(v),
  'object.keyql.limit': v => _validations.object(v),
  buffer: v => Buffer.isBuffer(v),
  any: v => true
};

// Unlike validations, santizizations will throw errors if failed
const _sanitizations = {
  ...
  'object.keyql.query': v => KeyQL.validateQueryObject(v),
  'object.keyql.limit': v => KeyQL.validateLimit(v)
};
