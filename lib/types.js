...
const validate = (type, v, nullable, schemas, options, range) => {
  if (schemas && !Array.isArray(schemas)) {
    throw new Error(`Argument schemas must be an Array`)
  }
  return (nullable && v === null) ||
    _validations[type](v, schemas, options, range);
};
...
const _sanitizations = {
  number: (v, range) => {
    if (range) {
      if (v < range.min) {
        throw new Error(`must be greater than or equal to ${range[0]}`);
      } else if (v > range.max) {
        throw new Error(`must less than or equal to ${range[1]}`);
      }
    }
    return v;
  },
