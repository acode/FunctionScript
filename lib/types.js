...
const validate = (type, v, nullable, schema) => {
  return (nullable && v === null) ||
    _validations[type](v, schema);
};

const _validations = {
  string: (v, options = []) => typeof v === 'string',
  number: (v, options = []) => typeof v === 'number' && v === v,
  float: (v, options = []) => parseFloat(v) === v,
  integer: (v, options = []) => parseInt(v) === v && v >= Number.MIN_SAFE_INTEGER && v <= Number.MAX_SAFE_INTEGER,
  boolean: (v, options = []) => typeof v === 'boolean',
  object: (v, schema = []) => {
  ...
  'object.keyql.query': (v, options) => _validations.object(v),
