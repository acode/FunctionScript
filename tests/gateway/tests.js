...
module.exports = (expect) => {
  ...
  it('Should register an error in the resolve step with type SubscriptionUpgradeRequiredError', done => {

    let originalResolveFn = FaaSGateway.resolve;
    FaaSGateway.resolve = (req, res, buffer, callback) => {
      let error = new Error('You are not allowed to access this API.');
      error.subscriptionUpgradeRequiredError = true;
      return callback(error);
    };
    ...
    request('POST', {}, '/my_function/', {}, (err, res, result) => {
      ...
      expect(res.statusCode).to.equal(401);
      expect(result.error).to.exist;
      expect(result.error.type).to.equal('SubscriptionUpgradeRequiredError');
      done();
