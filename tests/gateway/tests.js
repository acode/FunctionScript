...
module.exports = (expect) => {
  ...
  it('Should accept buffer in params', done => {
    request('POST', {}, '/buffer_in_params/', {
      b: Buffer.from('This is a buffer')
    },
    (err, res, result) => {
  ...
  it('Should accept buffer nested in params', done => {
    request('POST', {}, '/buffer_nested_in_params/', {
      upload: {
        attributes: {
          name: 'sample.txt',
          parent: {
        ...
        file: Buffer.from('This is a buffer')
      }
    },
    (err, res, result) => {
  ...
  it('Should give ParameterError if non-buffer passed in for buffer param', done => {
    request('POST', {}, '/buffer_in_params/', {
      b: {data: 'some data'}
    },
    (err, res, result) => {
      ...
      expect(result.error.type).to.equal('ParameterError');
      expect(result.error.details).to.exist;
      expect(result.error.details.b).to.exist;
      expect(result.error.details.b.expected).to.exist;
      expect(result.error.details.b.expected.type).to.equal('buffer');
      expect(result.error.details.b.actual).to.exist;
      expect(result.error.details.b.actual.type).to.equal('object');
      expect(result.error.details.b.actual.value).to.deep.equal({data: 'some data'});
      done();

    });
  });

  it('Should give ParameterError if non-buffer passed in for nested buffer param', done => {
    request('POST', {}, '/buffer_nested_in_params/', {
      upload: {
        attributes: {
          name: 'sample.txt',
        ...
        file: {type: 'Buffer', data: '123', b: 4444, c: 555}
      }
    },
    (err, res, result) => {
      ...
      expect(result.error.type).to.equal('ParameterError');
      expect(result.error.details).to.exist;
      expect(result.error.details.upload).to.exist;
      expect(result.error.details.upload.expected).to.exist;
      expect(result.error.details.upload.expected.type).to.equal('object');
      expect(result.error.details.upload.expected.schema).to.exist;
      expect(result.error.details.upload.expected.schema).to.have.length(2);
      expect(result.error.details.upload.expected.schema[1].name).to.equal('file');
      expect(result.error.details.upload.expected.schema[1].type).to.equal('buffer');
      expect(result.error.details.upload.actual).to.exist;
      expect(result.error.details.upload.actual.value).to.exist;
      expect(result.error.details.upload.actual.value.file).to.exist;
      expect(result.error.details.upload.actual.value.file).to.deep.equal({"type": "Buffer", "data": "123", "b": 4444, "c": 555});
      done();
